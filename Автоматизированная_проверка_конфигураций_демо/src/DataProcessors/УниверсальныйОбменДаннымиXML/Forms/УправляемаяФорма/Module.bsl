...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокФормы = СтрШаблон(НСтр("ru='Универсальный обмен данными в формате XML (%1)'"),
		ВерсияОбъектаСтрокойНаСервере());
...
Процедура ИмяФайлаДанныхПриИзменении(Элемент)
	...
	Если (НЕ ПустаяСтрока(Объект.ИмяФайлаПравилОбмена))
		И (СтрСравнить(СокрЛП(Объект.ИмяФайлаПравилОбмена), СокрЛП(Объект.ИмяФайлаОбмена)) = 0) Тогда
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Если НЕ Объект.НепосредственноеЧтениеВИБПриемнике И НЕ ЭтоКлиент Тогда
		...
		Если СтрСравнить(СокрЛП(Объект.ИмяФайлаПравилОбмена), СокрЛП(Объект.ИмяФайлаОбмена)) = 0 Тогда
			
			ТекстПредупреждения =
				НСтр("ru='Файл правил обмена не может совпадать с файлом данных. Выберите другой файл для выгрузки данных.'");
