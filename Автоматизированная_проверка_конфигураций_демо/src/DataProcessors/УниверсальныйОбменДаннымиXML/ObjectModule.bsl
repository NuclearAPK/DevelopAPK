...
#Область ВспомогательныеПеременныеМодуляДляНаписанияАлгоритмов_ОбщиеДляВыгрузкиИЗагрузки
...
Перем МенеджерыДляПлановОбмена Экспорт; // Соответствие для хранения менеджеров для планов обмена.
...
Перем ФлагКомментироватьОбработкуОбъектов Экспорт; // Флаг для определения, надо ли комментировать обработку объектов.
...
Перем СчетчикВыгруженныхОсобенностей; // Переменная считает количество выгруженных записей РС "Найденные ошибки".
...
#Область ПеременныеОбработокОбмена_ОбщиеДляВыгрузкиИЗагрузки
...
Перем одТипУзлаXML_КонецЭлемента  Экспорт; // Значение, равное ТипУзлаXML.КонецЭлемента.
Перем одТипУзлаXML_НачалоЭлемента Экспорт; // Значение, равное ТипУзлаXML.НачалоЭлемента.
Перем одТипУзлаXML_Текст          Экспорт; // Значение, равное ТипУзлаXML.Текст.

Перем ЗначениеПустаяДата Экспорт; // Значение, равное пустой дате - Дата('00010101').

Перем одСообщения;             // Соответствие. Ключ - код ошибки,  Значение - описание ошибки.

Перем мСписокМакетовПравилОбмена Экспорт; // Список макетов этой обработки.

#КонецОбласти

#Область ПеременныеОбработкиВыгрузки
...
#Область ПеременныеОбработкиЗагрузки

Перем мСчетчикЗагруженныхОбъектов Экспорт;  // Число - счетчик загруженных объектов.
Перем мАтрибутыФайлаОбмена Экспорт;         // Структура. После открытия файла содержит атрибуты файла обмена согласно формату.

Перем ЗагруженныеОбъекты Экспорт;           // Соответствие: Ключ - Нпп объекта в файле, Значение - ссылка на загруженный объект.
Перем ЗагруженныеГлобальныеОбъекты Экспорт; // Соответствие загруженных глобальных объектов.
Перем ЧислоХранимыхЗагруженныхОбъектов Экспорт; // число хранимых загруженных объектов,
                                                // после которого Соответствие ЗагруженныеОбъекты очищается.
Перем ЗапоминатьЗагруженныеОбъекты Экспорт;     // Флаг, определяющий, надо ли запоминать загруженные объекты.
...
Перем мОбработкаДляЗагрузкиДанных Экспорт; // Обработка для загрузки данных, подключаемая в другой базе.

Перем мСоответствиеПустыхЗначенийТипов;
Перем мСоответствиеОписаниеТипов;

Перем мБылиПрочитаныПравилаОбменаПриЗагрузке Экспорт; // Флаг, определяющий, были ли прочитаны правила обмена при загрузке.
...
Перем СобытияПослеЗагрузкиПараметров Экспорт; // Структура, содержащая события после загрузки параметров.
...
#Область ПроцедурыИФункцииДляРаботыСОбъектомDOM_XML
...
#Область ПроцедурыИФункцииДляРаботыСФайломОбмена
...
#Область ПроцедурыИФункцииДляРаботыСПротоколомОбмена
...
Функция ПолучитьСтруктуруЗаписиПротокола(КодСообщения = "", СтрокаОшибки = "") Экспорт
	...
	СтруктураОшибки.Вставить("КодСообщения");
...
// Параметры:
//  ПравилаОбмена  - Объект типа XMLЧтение.
//  Значения       - соответствие значений объекта источника строковым представлениям объекта приемника.
...
// Параметры:
//  ПравилаОбмена  - Объект типа XMLЧтение.
//  ЗаписьXML      - Объект типа XMLЗапись - правила, сохраняемые в файл обмена и используемые при загрузке данных.
//
	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "ПоискПоТабличнымЧастям" Тогда
			...
			Для Номер = 1 По СтрЧислоСтрок(Значение) Цикл
				...
				СтрокаТаблицы.ПоляПоискаТЧ = СтрРазделить(СтрокаПоиска, ",", Ложь);
...
Процедура ЗагрузитьПравилаОбменаДляЭкспортаОбработчиков()
	...
	Иначе // файл данных
		...
		// При наличии флага обработка потребует перечитать правила при попытке выгрузки данных.
...
//  Если режим отладки алгоритмов - "алгоритмы отлаживать как процедуры",
//  то в конструктор добавляется структура "Алгоритмы".
...
Процедура ДобавитьВПотокПолныйОбработчик(Результат, ИнтерфейсОбработчика, Обработчик)
	...
	Для Индекс = 1 По СтрЧислоСтрок(Обработчик) Цикл
		...
		// В режиме отладки алгоритмов "Интеграция кода" вставляем код алгоритмов непосредственно в код обработчика.
		// Код алгоритма вставляем взамен его вызова. В коде алгоритмов уже учтена вложенность алгоритмов друг в друга.
		Если РежимОтладкиАлгоритмов = мРежимыОтладкиАлгоритмов.ИнтеграцияКода Тогда
...
Процедура ДобавитьВПотокСлужебныйКод(Результат, ИмяОбласти)
	...
	// Исключаем последний перевод строки.
	Текст = Сред(Текст, 1, СтрДлина(Текст));
...
Функция ЗаменитьВызовыАлгоритмовКодомЭтихАлгоритмовВОбработчике(Обработчик, АлгоритмВладелец, МассивЗапрошенныхЭлементов, Знач СтрокаПрефикса = "")
	...
	Для Индекс = 1 По СтрЧислоСтрок(Обработчик) Цикл
		...
		Если АлгоритмыОбработчика.Количество() <> 0 Тогда // В этой строке есть вызовы алгоритмов.
			...
			// Разворачиваем полный код каждого алгоритма, который был вызван из строки "СтрокаОбработчика".
			Для Каждого Алгоритм Из АлгоритмыОбработчика Цикл
...
// Формирует интерфейс глобального обработчика конвертации
// (уникальное имя процедуры с параметрами соответствующего обработчика).
//
// Параметры:
...
// Параметры:
//  ПКО - Строка таблицы значений    - содержит правило конвертации объекта.
//  ПравилаКонвертацииСвойствОбъекта - ТаблицаЗначений - содержит правила конвертации свойств или
//                                                       группы свойств объекта из правила ПКО.
//  ПКГС - Строка таблицы значений   - содержит правило конвертации группы свойств.
//
...
Функция СоздатьНовыйОбъект(Тип, СвойстваПоиска, Объект = Неопределено, ЗаписыватьОбъектСразуПослеСоздания = Истина,
	...
	// Если Владелец не установлен, то нужно добавить поле в возможные поля поиска,
	// а в событии ПОЛЯПОИСКА указать поля без Владельца, если по нему поиск реально не нужен.
...
Функция НайтиИлиСоздатьОбъектПоСвойству(СтруктураСвойств, ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать,
	ИмяТипаОбъекта, СвойствоПоиска, ЗначениеСвойстваПоиска, ОбъектНайден, СоздаватьНовыйЭлементЕслиНеНайден = Истина,
	НайденныйИлиСозданныйОбъект = Неопределено, РежимПоискаОсновногоОбъекта = Ложь, СвойстваОбъектаМодифицированы = Ложь,
	...
	ОбъектНайденИлиСоздан = НЕ (Объект = Неопределено ИЛИ Объект.Пустая());
	
	ОбъектНайден = НЕ Объект.Пустая();
	
	Если (НЕ ОбъектНайденИлиСоздан) И СоздаватьНовыйЭлементЕслиНеНайден Тогда
...
Процедура ПрочитатьСвойстваПоискаИзФайла(СвойстваПоиска, СвойстваПоискаНеЗамещать, ИнформацияОТипах,
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если (ИмяУзла = "Свойство") ИЛИ (ИмяУзла = "ЗначениеПараметра") Тогда
			...
			ПоискПоДатеНаРавенство = ПоискПоДатеНаРавенство ИЛИ одАтрибут(ФайлОбмена, одТипБулево, "ПоискПоДатеНаРавенство");
...
Функция НайтиЭлементПоСвойствамПоиска(ТипОбъекта, ИмяТипаОбъекта, СвойстваПоиска, СтруктураСвойств,
	...
	// Нужно искать только по тем свойствам, которые имеются в строке имен свойств.
	// Если там пусто, то по всем имеющимся свойствам поиска.
...
Процедура ОбработатьУстановкуСвойствПоискаУОбъекта(УстанавливатьУОбъектаВсеСвойстваПоиска, ТипОбъекта, СвойстваПоиска,
	...
	// Если было что-то изменено, тогда перезаписываем объект.
...
Функция ОбработатьПоискОбъектаПоСтруктуре(НомерОбъекта, ТипОбъекта, СозданныйОбъект, РежимПоискаОсновногоОбъекта,
	СвойстваОбъектаМодифицированы, ОбъектНайден, ЭтоГлобальныйНомер, ПараметрыОбъекта = Неопределено)
...
//
Функция НайтиОбъектПоСсылке(ТипОбъекта, СвойстваПоиска = "", СвойстваПоискаНеЗамещать = "", ОбъектНайден = Истина,
	СозданныйОбъект = Неопределено, НеСоздаватьОбъектЕслиНеНайден = Неопределено, РежимПоискаОсновногоОбъекта = Ложь,
	СвойстваОбъектаМодифицированы = Ложь, НППГлобальнойСсылки = 0, НппСсылки = 0,
	ИзвестнаяСсылкаУникальногоИдентификатора = Неопределено, ПараметрыОбъекта = Неопределено, ИмяПКО = "")
	...
	Если ЗапоминатьЗагруженныеОбъекты Тогда
		...
		// Есть номер по порядку из файла, ищем по нему.
	...
	ИначеЕсли (СвойствоУникальныйИдентификатор <> Неопределено) Тогда
		...
		// Не всегда нужно создавать новый элемент по уникальному идентификатору, возможно, нужно продолжить поиск.
		...
		СсылкаНаОбъект = НайтиИлиСоздатьОбъектПоСвойству(СтруктураСвойств, ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать,
			...
			АвтоматическиСоздаватьНовыйОбъект, СозданныйОбъект, РежимПоискаОсновногоОбъекта, СвойстваОбъектаМодифицированы,
			ИзвестнаяСсылкаУникальногоИдентификатора, НппСсылки, НППГлобальнойСсылки, ПараметрыОбъекта);
	...
	Если РежимПоискаОсновногоОбъекта
		...
		И (ИмяТипаОбъекта = "Документ" ИЛИ ИмяТипаОбъекта = "Задача" ИЛИ ИмяТипаОбъекта = "БизнесПроцесс") Тогда
	...
	Если НЕ ЗначениеЗаполнено(СсылкаНаОбъект)
		...
		ИначеЕсли НЕ НеСоздаватьОбъектЕслиНеНайден Тогда
			...
			СсылкаНаОбъект = СоздатьНовыйОбъект(ТипОбъекта, СвойстваПоиска, СозданныйОбъект, НЕ РежимПоискаОсновногоОбъекта,,
				ИзвестнаяСсылкаУникальногоИдентификатора, НппСсылки, НППГлобальнойСсылки,, УстанавливатьУОбъектаВсеСвойстваПоиска);
...
//
Процедура УстановитьСвойстваЗаписи(Объект, Запись, ИнформацияОТипах, ПараметрыОбъекта, ИмяВетки, НомерЗаписи,
...
Процедура ДополнитьСтекНеЗаписанныхОбъектов(Нпп, ГНпп, Объект, ИзвестнаяСсылка, ТипОбъекта, ПараметрыОбъекта)
	...
	Структура.Вставить("ИзвестнаяСсылка", ИзвестнаяСсылка);
	Структура.Вставить("ТипОбъекта", ТипОбъекта);
	Структура.Вставить("ПараметрыОбъекта", ПараметрыОбъекта);
	
	мГлобальныйСтекНеЗаписанныхОбъектов.Вставить(НомерДляСтека, Структура);
...
Процедура ПровестиГенерациюКодаНомераПриНеобходимости(ГенерироватьНовыйНомерИлиКодЕслиНеУказан, Объект, ИмяТипаОбъекта,
	...
	Если НЕ ГенерироватьНовыйНомерИлиКодЕслиНеУказан
		...
		// Если номер не нужно генерировать, или не в режиме обмена данными, то ничего не нужно делать -
		// платформа сама все сгенерирует.
		Возврат;
	КонецЕсли;
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Свойство"
			...
			Если НЕ ЭтоПараметрДляОбъекта
				...
				// Так и не смогли создать объект в событии - создаем его отдельно.
				Если Объект = Неопределено Тогда
				...
				Если ИмяТипаОбъекта = "РегистрСведений" Тогда
					...
					Если ЗаписыватьРегистрыНаборамиЗаписей Тогда
						...
						НужноПроверитьДанныеДляВременногоНабора =
							(ЗаписыватьВИнформационнуюБазуТолькоИзмененныеОбъекты И НЕ НужноЗаписатьОбъект)
						...
						// Нужно установить отбор у регистра.
...
//  ВходящиеДанные       - произвольные вспомогательные данные, передаваемые правилу для выполнения конвертации.
//  ИсходящиеДанные      - произвольные вспомогательные данные, передаваемые правилам конвертации объектов свойств.
...
Процедура ВыгрузитьГруппуСвойств(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКГС, УзелКоллекцииСвойств,
	...
	Если ВыгружатьГруппуЧерезФайл Тогда
		...
		Исключение
			...
			СтрокаСообщенияОбОшибке = ЗаписатьВПротоколВыполнения(8);
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(1000, ОписаниеОшибки(),
				ИмяФайлаЗаписей + ": Ошибка при создании временного файла для выгрузки данных");
...
Функция ОпределитьТипДанныхДляПриемника(Значение)
	...
	// если правила нет, то ""; если есть, тогда то, что нашли, оставляем.
...
Функция ВыгрузитьПоПравилу(Источник = Неопределено, Приемник = Неопределено, ВходящиеДанные = Неопределено,
	...
	Если Не НеНужноВыгружатьПоСоответствиюЗначений Тогда
		...
		// Если нет объекта в соответствии значений, то выгружаем его штатным образом.
	...
	Если ПКО.Источник = Тип("РегистрСведенийЗапись.НайденныеОшибки") Тогда
		СчетчикВыгруженныхОсобенностей = СчетчикВыгруженныхОсобенностей + 1;
	КонецЕсли;
	...
	Если КоличествоОбработанныхОбъектовДляОбновленияСтатуса > 0
		...
		Попытка
			СтрокаСостояния = СтрШаблон(НСтр("ru='Выгрузка объектов "" %1"". Выгружено: %2 Объект: %3'"), Строка(Источник),
				Строка(мСчетчикВыгруженныхОбъектов), ИмяМетаданных);
			Состояние(СтрокаСостояния);
		Исключение
			СтрокаСостояния = СтрШаблон(НСтр("ru='Выгрузка объектов """". Выгружено: %1 Объект: %2'"),
				Строка(мСчетчикВыгруженныхОбъектов), ИмяМетаданных);
			Состояние(СтрокаСостояния);
		КонецПопытки;
...
Функция ПолучитьРезультатЗапросаДляВыгрузкиОчисткиДанных(Свойства, ИмяТипа, ВыборкаДляОчисткиДанных = Ложь,
	...
	ИначеЕсли ИмяТипа = "РегистрСведений" Тогда
		...
		ВыгрузкаОсобенностей = (Свойства.ТипСсылкиСтрокой = "РегистрСведенийЗапись.НайденныеОшибки");
		
		Непериодический = НЕ Свойства.Периодический;
		ПодчиненныйРегистратору = Свойства.ПодчиненныйРегистратору;
		
		УсловияЗапроса = ПолучитьСтрокуОграниченияПоДатеДляЗапроса(Свойства, ИмяТипа, "ОбъектДляВыгрузки", ВыборкаДляОчисткиДанных);
		
		Если ВыгрузкаОсобенностей Тогда
			...
			УсловияЗапроса = УсловияЗапроса + "ОбъектДляВыгрузки.Объект.Владелец.Владелец = &Конфигурация
			|	И ОбъектДляВыгрузки.Состояние = &Состояние";
...
Функция ВыполнитьВыгрузкуИзмененныхДанныхДляУзлаОбмена(УзелОбмена, МассивПравилКонвертации, СтруктураДляУдаленияРегистрацииИзменений)
	...
	Попытка
		...
		// Дополняем массив только теми метаданными, по которым есть правила выгрузки.
		// Остальные метаданные нас не интересуют.
		...
		Пока ВыборкаИзменений.Следующий() Цикл
			...
			Если КоличествоОбработанныхОбъектовДляОбновленияСтатуса > 0
				...
				#Если Клиент Тогда
				Попытка
					СтрокаСостояния = СтрШаблон(("ru = 'Запись изменений для узла ""%1"". Прочитано: %2 Объект: %3'"),
						УзелОбмена.Наименование, Строка(КоличествоНайденныхДляЗаписиОбъектов), ИмяМетаданных);
					Состояние(СтрокаСостояния);
				Исключение
				КонецПопытки;
				#КонецЕсли
...
//
// Параметры:
//   ТолькоПрочитатьШапку - Булево - если Истина, то после прочтения шапки файла обмена (корневой узел), файл закрывается.
//   ДанныеФайлаОбмена    - Строка - данные файла обмена.
//
...
// Заполняет переданную таблицу значений типами объектов метаданных для удаления,
// на которые есть право доступа для удаления.
//
...
Процедура УстановитьПометкиРодителей(ТекСтрока, Реквизит) Экспорт
	...
	Если Реквизит = "ИспользоватьОтбор" Тогда
		...
		Для Каждого Строка Из Родитель.Строки Цикл
			...
			Если (Строка[Реквизит] = 0) И (Строка.НастройкиПостроителя <> Неопределено) Тогда
...
Процедура ЗагрузитьПравилаОбмена(Источник = "", ТипИсточника = "XMLФайл") Экспорт
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Исключение
			...
			Текст = СтрШаблон(НСтр("ru='Обработчик: ""ПослеЗагрузкиПравилОбмена"": %1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
// Открывает форму для формирования файла с обработчиками выгрузки,
// когда правила обмена загружаются из макета обработки.
//
...
Функция ПолучитьИмяФайлаОбмена(ИмяКаталога, ИмяФайла) Экспорт
	...
	Если Не ПустаяСтрока(ИмяФайла) Тогда
		...
		Возврат ИмяКаталога + ?(СтрЗаканчиваетсяНа(ИмяКаталога, "\"), "", "\") + ИмяФайла;
...
//
// Параметры:
//  ИнформацияДляЗаписиВФайл   - Строка(XML) - строка с данными.
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	СчетчикВыгруженныхОсобенностей = 0;
	...
	Параметры.Вставить("Конфигурация", Конфигурация);
	Параметры.Вставить("КонфигурацияНаименование", КонфигурацияНаименование);
	...
	Сообщение = СтрШаблон(НСтр("ru='Выгружено особенностей: %1'"), СчетчикВыгруженныхОсобенностей);
	Сообщить(Сообщение);
...
Процедура ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеджерыДляПлановОбмена, ОбъектМД, ИмяТипа, Менеджер,
	...
	СтруктураДляПланаОбмена = Новый Структура;
	СтруктураДляПланаОбмена.Вставить("Имя", Имя);
	СтруктураДляПланаОбмена.Вставить("ТипСсылки", ТипСсылки);
	СтруктураДляПланаОбмена.Вставить("ЭтоСсылочныйТип", Истина);
	СтруктураДляПланаОбмена.Вставить("ЭтоРегистр", Ложь);
...
Процедура ДополнитьМассивМенеджеровТипомРегистра(Менеджеры, ОбъектМД, ИмяТипа, Менеджер, ПрефиксИмениТипаЗапись, ПрефиксИмениТипаВыборка)
	...
	СтруктураДляПланаОбмена.Вставить("Имя", Имя);
	СтруктураДляПланаОбмена.Вставить("ТипСсылки", ТипСсылки);
	СтруктураДляПланаОбмена.Вставить("ЭтоСсылочныйТип", Ложь);
	СтруктураДляПланаОбмена.Вставить("ЭтоРегистр", Истина);
...
Процедура СоздатьСтруктуруКонвертации()
	...
	Конвертация.Вставить("ПередКонвертациейОбъекта");
	Конвертация.Вставить("ПередЗагрузкойОбъекта");
	Конвертация.Вставить("ПослеЗагрузкиОбъекта");
	Конвертация.Вставить("ПередЗагрузкойДанных");
	Конвертация.Вставить("ПослеЗагрузкиДанных");
...
Функция ПодключитьсяКИнформационнойБазе(СтруктураПодключения, СтрокаСообщенияОбОшибке = "")
...
//
// Параметры
//  ИмяФайла     - Строка - строка, содержащая имя файла, неважно с именем каталога или без.
//
// Возвращаемое значение:
...
Функция ВыполнитьПодключениеКИБПриемнику() Экспорт
	...
	ОбъектПодключения = ПодключитьсяКИнформационнойБазе(СтруктураПодключения);
